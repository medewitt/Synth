dataprep <-
  ...
  {
    if (is.data.frame(foo) == FALSE) {
      stop("\n No data.frame supplied in foo.\n")
    }
    
    # This presents tibbles from breaking all of the logic below:
    # Ex foo <- as_tibble(mtcars)
    #    mode(foo[, "hp"]) = "list" which breaks everything below
    ...
    if (is.null(unit.variable) == TRUE ||
        mode(foo[, unit.variable]) != "numeric")
    {
      stop("\n unit.variable not found as numeric variable in foo.\n")
    }
    if (length(unit.variable) != 1) {
      stop("\ Please specify only one unit.variable\n")
    }
    ...
    if (is.null(time.variable) == TRUE ||
        mode(foo[, time.variable]) != "numeric")
    {
        ...
        is.na(unit.names.variable) == FALSE)
    {
      ...
      if (is.na(unit.names.variable) == TRUE ||
          mode(foo[, unit.names.variable]) != "character")
      {
      ...
      if (length(tr.id) != 1)
      {
        stop(
          paste(
            ...
            "refers to multiple numbers in unit.variable"
          )
        )
      }
          ...
          if (length(co.id) != 1)
          {
            stop(
              paste(
                "\n control unit number",
                i,
                " refers to multiple names in unit.names.variable"
              )
            )
          }
          co.store <- c(co.store, co.id)
    ...
    foo[, time.variable] <-
      as.numeric(as.character(foo[, time.variable]))
    foo[, unit.variable] <-
      as.numeric(as.character(foo[, unit.variable]))
    foo <-
      foo[order(foo[, unit.variable], foo[, time.variable]), ]
    
    # Get rows
    treatment.rows <-
      which(foo[, unit.variable] %in% treatment.identifier)
    control.rows   <-
      which(foo[, unit.variable] %in% controls.identifier)
    
    # check if panel is unbalanced  
    # (possible cases where this too restrictive, but imposes discipline)
    
    balcheck <-
      table(foo[c(control.rows, treatment.rows), unit.variable],
    ...
    t.list <-
      list(time.predictors.prior, time.optimize.ssr, time.plot)
    ...
    for (i in 1:3)
    {
      ...
      {
        stop(paste(
          "\n specificy at least one period in time",
          names(t.list[i]),
          "\n"
        ))
      }
    ...
    if (is.null(predictors) == FALSE)
    {
      ...
      if (mode(predictors) == "character")
      {
        ...
        for (i in 1:length(predictors)) {
          pred.no <- c(pred.no, which(names(foo) == predictors[i]))
        }
        predictors <- pred.no
      }
      # X1 matrix for treated
      X1 <-
        data.frame(foo[intersect(treatment.rows,
                                 time.predictors.prior.rows),
                       predictors])
      ...
      for (i in 1:ncol(X1))
      {
        if (sum(is.na(X1[, i])) == nrow(X1))
        {
          stop(
            paste(
              "\n predictor",
              names(X1)[i],
              "has missing data for all periods in time.predictors.prior\n"
            )
          )
        }
        ...
        for (j in 1:nrow(X1))
        {
          if (is.na(X1[j, i])) {
            cat(
              paste(
                "\n Missing data- treated unit; predictor:",
                names(X1)[i],
                "; for period:",
                rownames(X1)[j],
                "\n",
      ...
      # if no regular predictors are specified, pass a void matrix to special predictors
    ...
    if (is.null(predictors) == FALSE)
    {
      ...
      checknames <-
        rep(time.predictors.prior, length(controls.identifier))
    ...
    if (is.null(special.predictors) == FALSE)
    {
      ...
      for (i in 1:length(special.predictors))
      {
        ...
        if (length(special.predictors[[i]]) != 3)
        {
          stop(
            paste(
              "\n special.predictor number",
              i,
              "is misspecified (requires predictor name, time periods, and name of operation\n"
            )
          )
        }
        ...
        if (mode(foo[, sp.name]) != "numeric")
        {
          stop(
            paste(
              "\n special predictor named ",
              sp.name,
              "not found as numeric variable in foo \n"
            )
          )
        }
        ...
        if (mode(sp.time) != "numeric")
        {
          stop(
            paste(
              "\n for special predictor:",
              sp.name,
              " (which is special.predictor number",
              i,
              ") the time period is misspecified\n"
            )
          )
        }
        if (sum(duplicated(sp.time)) > 0)
        {
          stop(
            paste(
              ...
              " (which is special.predictor number",
              i,
              ") time period contains duplicates\n"
            )
          )
        }
        if (length(sp.time) < 1)
        {
          stop(
            paste(
              ...
              " (which is special.predictor number",
              i,
              ") specify at least on time period\n"
            )
          )
        }
    ...
    Z1 <- foo[intersect(treatment.rows,
                        time.optimize.ssr.rows),
              dependent]
    ...
    for (i in 1:length(Z1))
    {
      if (is.na(Z1[i]))
      {
        stop(
          paste(
            "\n treated unit: dependent variable is missing for period",
            rownames(Z1)[i],
            "in time.optimize.ssr.rows \n"
          )
        )
      }
    }
    ...
    Z0 <-
      as.matrix(foo[intersect(control.rows,
                              time.optimize.ssr.rows),
                    c(dependent)])
    ...
    Y1plot <-
      foo[intersect(treatment.rows, time.plot.rows), dependent]
    ...
    Y0plot <- as.matrix(foo[intersect(control.rows, time.plot.rows),
                            c(dependent)])
    ...
    names.and.numbers <-
      data.frame(
        c(treatment.identifier.name, controls.identifier.name),
        c(treatment.identifier, controls.identifier)
      )
    
    
    
    names(names.and.numbers) <- c("unit.names",
                                  "unit.numbers")
